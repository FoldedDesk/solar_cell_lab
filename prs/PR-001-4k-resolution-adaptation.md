...
## 最近增补（2026-05-24）
- Windows 分辨率切换稳定性修复：
  - 切换分辨率时先尝试退出最大化（`state("normal")` / `attributes("-zoomed", False)`）。
  - `geometry` 设置后追加一次短延时重设，规避部分机型首帧被系统覆盖导致“看起来未生效”。
  - 设置菜单增加“当前窗口 WxH”只读项，便于现场验证是否切换成功。
- 实验二距离弹窗“记录当前数据”按钮宽度修复：
  - 统一放入独立整行容器并 `fill=tk.X + expand=True`，避免在部分分辨率/DPI 下宽度偏窄。
- 计时栏显示裁切修复：
  - 计时栏高度改为随屏幕自适应（带上下限），并禁用容器自动收缩。
  - 计时字体按栏高动态计算，避免低分辨率或高 DPI 下黑底数字被截断。
